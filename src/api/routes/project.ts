        ...
        route.get(
          ...
          async (req: Request, res: Response, next: NextFunction) => {
            ...
            try {
              ...
              console.log ( " received subject is " + projectType)
              const projectServiceInstance = Container.get(ProjectService);
              const { project} = await projectServiceInstance.GetProjectsByDomain(projectType); /* service get ngos with specific domain*/
              return res.status(201).json({ project });
            } catch (e) {
              logger.error('🔥 error: %o', e);
              return next(e);
        ...
        route.get(
          ...
          async (req: Request, res: Response, next: NextFunction) => {
            ...
            try {
              ...
              const projectServiceInstance = Container.get(ProjectService);
              const { project} = await projectServiceInstance.GetProjectsByCountry(country); /* service get ngos with specific domain*/
              return res.status(201).json({ project });
            } catch (e) {
              logger.error('🔥 error: %o', e);
              return next(e);
