...
/* Custom API to a specific ngo*/
public async GetNgoById( id: string): Promise<{ ngo: INgoProfile[] }> {
  try {
    ...
    const ngoProfileRecord = await this.ngoProfileModel.find({_id: id});
    this.logger.silly('Generating JWT');
...
public async GetNgoByuserId( id: string): Promise<{ ngo: INgoProfile[] }> {
  try {
    ...
    this.logger.silly('ngo');
    this.logger.silly('retrieving donor db record');
    const ngoProfileRecord = await this.ngoProfileModel.find({userId: id});
    this.logger.silly('Generating JWT');
  ...
  //api which returns userID by ngoID
  //GetUserIdByNgoId(id)
  public async GetUserIdByNgoId( id: string): Promise<{ ngo: INgoProfile[] }> {
    try {
